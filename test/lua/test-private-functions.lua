...
describe(
  'Test private functions', function()
    ...
    describe(
      'Function “luafy_options()”', function()
        it(
          'Key containing white spaces', function()
            assert.are.same(
              luakeys.luafy_options(
                { ['key 1'] = 'one', ['key 2'] = 'two' }
              ), { key_1 = 'one', key_2 = 'two' }
            )
          end
    ...
    describe(
      'Function “normalize()”', function()
        ...
        it(
          'Unchanged', function()
            assert.are.same(
              normalize({ one = ' one ' }, get_options()), { one = ' one ' }
            )
          end
        )

        describe(
          'Option standalone_as_true', function()
            it(
              'true', function()
                assert.are.same(
                  normalize(
                    { 'standalone' }, get_options({ standalone_as_true = true })
                  ), { standalone = true }
                )
              end
            )
    ...
    describe(
      'Function “normalize_parse_options()”', function()
        it(
          'No options', function()
            assert.is.same(
              luakeys.normalize_parse_options(), {
    ...
    describe(
      'Function “unpack_single_valued_array_table()”', function()
        local unpack = luakeys.unpack_single_valued_array_table

        it(
          'unpacked: single string', function()
        ...
        it(
          'Not unpacked: two values', function()
            assert.is
              .same(unpack({ 'one', 'two' }, get_options()), { 'one', 'two' })
          end
        )

        it(
          'Not unpacked: nested table', function()
    ...
    describe(
      'Function “visit_parse_tree()”', function()
        ...
        it(
          'Change the value', function()
            ...
            assert.is.same(
              visit({ l1 = { l2 = 1 } }, callback_func), { l1 = { l2 = 2 } }
            )
            assert.has_error(
              function()
        ...
        it(
          'Return key and value unchanged', function()
            assert.is.same(
              visit(
                { l1 = { l2 = 1 } }, function(key, value)
                  return key, value
    ...
    describe(
      'Function “merge_tables”', function()
