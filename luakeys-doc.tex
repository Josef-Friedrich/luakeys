...
\latex{\LuakeysSaveClassOptions{<identifier>}}
...
% Schlüssel-Wert-Paare über dieses Makro zu verarbeiten.
It is therefore not possible to process nested hierarchical key-value
pairs via this macro.
...
\noindent
% Mit einem kleinen Trick kann anscheinend der LaTeXs
% Normalisierungsprozess der Optionenzeichenkette umgangen werden,
% sodass auch verschachtelte Schüsselwertpaar erkannt werden können.
Apparently, the \LaTeX’s normalization process of the class options
string can be circumvented with a little trick, so that nested key value
pairs can also be recognized:
% Zwischen dem Istgleich-Zeichen und der öffnenden geschweiftenklammern
% darf keine Leerzeichen stehen:
There must be no spaces between the equals sign and the opening curly
braces.
...
\noindent
% Steht zwischen dem Istgleich-Zeichen und der geschweiften Klammer ein
% Leerzeichen, so wird die Zeichkette übersetzt, als wenn überhaupt
% keine geschweiften Klammer vorhanden wäre.
If there is a space between the equals sign and the curly brace, the
string is translated as if there were no curly braces at all

\begin{multicols}{2}
\begin{minted}{latex}
\documentclass[
  test = { % with a whitespace between = and {
    key1,
    key2
  }
]{test-class}
...
\latex{\LuakeysSavePackageOptions{<identifier>}}

\noindent
% Auf die Optionen eines \LaTeX{}-Pakets kann über das Macro
% \latex{\@ptionlist{\@currname.\@currext}} zu gegriffen werden.
The options of a \LaTeX{} package can be accessed via the
\latex{\@ptionlist{\@currname.\@currext}}  macro.
