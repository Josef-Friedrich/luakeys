...
\InputLua[firstline=3,lastline=14]{export.lua}
...
\InputLua[firstline=5,lastline=61]{defs/all-attrs.lua}
...
% In der Tabelle \lua{utils} sind einige Hilfsfunktionen gebündelt, die
% bei der Weiterverarbeitung von Tabellen mit Schlüssel-Werte-Paar
% eingesetzt werden können.
The \lua{utils} table bundles some auxiliary functions that can be used
when processing tables with key-value pairs.

\InputLua[firstline=3,lastline=10]{utils/all.lua}
...
% \latex{\LuakeysClassOptions} zu gegriffen werden.
The options of a \LaTeX{} document class can be accessed via the
\latex{\LuakeysClassOptions} macro.
%
\latex{\LuakeysClassOptions} is an alias for

\begin{center}
\latex{\luaescapestring{\@raw@classoptionslist}}.
\end{center}

The macro \latex{\LuakeysSaveClassOptions{<identifier>}} saves the
option string in the value store of the luakeys module. Use
\lua{luakeys.get('<indentifer>')} to access the stored option string.
...
% \latex{\LuakeysPackageOptions} zu gegriffen werden.
The options of a \LaTeX{} package can be accessed via the
\latex{\LuakeysPackageOptions} macro.
%
\latex{\LuakeysPackageOptions} is an alias for

\begin{center}
\latex{\luaescapestring{\@ptionlist{\@currname.\@currext}}}.
\end{center}

The macro \latex{\LuakeysSavePackageOptions{<identifier>}} saves the
option string in the value store of the luakeys module. Use
\lua{luakeys.get('<indentifer>')} to access the the stored option string.
...
\changes{0.12.0}{2022/12/30}{
* Breaking change! Export a function that returns the Lua API instead
  of a table. Use “require('luakeys')()” instead of
  “require('luakeys')”.
* New macros \cmd{\LuakeysPackageOptions}, \cmd{\LuakeysClassOptions}
  and \cmd{\LuakeysSavePackageOptions}, \cmd{\LuakeysSaveClassOptions}.
* New option: “accumulated_result”.
* New data type: “list”.
