...
\begin{minted}{lua}
-- a single alias
local defintions = { key = { alias = 'k' } }
-- multiple aliases
local defintions = { key = { alias = { 'k', 'ke' } } }
local parse = luakeys.define(defintions)
...
value is passed to the key. Three parameters are passed when the
function is called:
...
\inputminted[firstline=4,lastline=16]{lua}{examples/docs/definitions/options/sub-keys.lua}
...
\inputminted[firstline=4,lastline=11]{lua}{examples/docs/functions/render.lua}
...
\begin{minted}{lua}
result = luakeys.parse('one,two,three', { naked_as_value = true })
