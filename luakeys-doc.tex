...
\section{How the package is included or loaded}
...
\lua{opts} & Options & \lua{ { no_error = false } }\\
\lua{opt} & Option & \lua{no_error}, \lua{naked_as_value}\\
...
\begin{center}
\begin{tabular}{ll}
\lua{result} & The result of the parse function \\
\lua{unknown} & A table of unknown, not definied key-value pairs \\
\lua{raw} & The raw result of the Lpeg grammar parser \\
\end{tabular}
\end{center}
...
\subsection{Function \texttt{parse(kv\_string, options)}: table}
\label{parse}
...
\subsubsection{Option “\texttt{definitions}”}
...
local result, unknown = parse('level1={level2={key=value,unknown=unknown}}')
luakeys.debug(result) -- { level1 = { level2 = { key = 'value' } } })
luakeys.debug(unknown) -- { level1 = { level2 = { unknown = 'unknown' } } })
\end{minted}
...
\InputLuaExample[firstline=5,lastline=44]{defs/all-attrs.lua}
...
\subsection{Function \texttt{render(tbl)}: string}
...
\subsection{Function \texttt{debug(tbl): void}}
...
\subsection{Function \texttt{get(identifier): table}}
