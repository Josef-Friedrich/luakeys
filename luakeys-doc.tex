...
\subsection{\texttt{parse(kv\_string, options, defaults)}: table}
\label{parse}

The function \lua{parse(kv_string, options)} is the main method of
...
\subsection{\texttt{define(defs, parse_options, defaults)}}
...
\begin{enumerate}
\item as stand-alone values also known as an array of strings.
\item as keys in a Lua table. The definition of the key-value pair is
then under these keys.
...
The code example below lists all the options that can be used to define
key-value pairs.

\begin{minted}{lua}
local defs = {
  key = {
    alias = { 'k', 'ke' }, -- or a single string: alias = 'k'
    always_present = false,
    choices = { 'one', 'two', 'three' },
    data_type = 'string', -- or boolean, integer,
    default = 'value',
    exclusive_group = 'name',
    l3_tl_set = 'my_token_list', --> \g_my_token_list_tl
    macro = 'MacroName', --> \MacroName
    match = '^%d%d%d%d%-%d%d%-%d%d$', -- See http://www.lua.org/manual/5.3/manual.html#6.4.1
    name = 'key', -- name of the key, can be omitted
    opposite_values = {
      [true] = 'show',
      [false] = 'hide'
    },
...
\subsubsection{Sub keys: \texttt{sub\_keys}}
...
\subsubsection{Opposite values: \texttt{opposite\_values}}
...
\subsubsection{\texttt{macro}: Store values in a \TeX macro}

The option \texttt{macro} stores the value in a macro.
...
\subsubsection{Alias: \texttt{alias}}
